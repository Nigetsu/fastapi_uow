...
class Task(Base):
    ...
    assignee_id: Mapped[uuid4 | None] = mapped_column(UUID(as_uuid=True), ForeignKey('users.id'))
    column_id: Mapped[uuid4 | None] = mapped_column(UUID(as_uuid=True), ForeignKey('columns.id'))
    sprint_id: Mapped[uuid4 | None] = mapped_column(UUID(as_uuid=True), ForeignKey('sprints.id'))
    board_id: Mapped[uuid4 | None] = mapped_column(UUID(as_uuid=True), ForeignKey('boards.id'))
    group_id: Mapped[uuid4 | None] = mapped_column(UUID(as_uuid=True), ForeignKey('groups.id'))
